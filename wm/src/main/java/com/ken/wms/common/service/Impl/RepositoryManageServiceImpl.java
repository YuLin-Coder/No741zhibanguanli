...
public class RepositoryManageServiceImpl implements RepositoryService {
    ...
    /**
     * 返回指定 repository address 的值班记录 支持查询分页以及模糊查询
     *
     * @param offset  分页的偏移值
     * @param limit   分页的大小
     * @param address 值班的地址
    ...
    /**
     * 返回指定 repository address 的值班记录 支持模糊查询
     *
     * @param address 值班名称
     * @return 结果的一个Map，其中： key为 data 的代表记录数据；key 为 total 代表结果记录的数量
     */
     ...
     * @return 若值班信息满足要求则返回true，否则返回false
     */
